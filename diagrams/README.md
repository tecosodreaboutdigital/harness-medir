...
**Why English.** Per `STANDARDS.md`'s `Languages` rule, new content is authored in English first, so these were drafted English-only, then translated once each article's prose was ready. Parts 3 and 4 are both now fully trilingual: each diagram above has been copied inline into `harness-p3.html` or `harness-p4.html`, re-IDed per language through the build scripts' `scope()` function (`p3d1` through `p3d5`, `p4d6` through `p4d10`), with every label translated and checked by rendering the diagram standalone in each language before merging it in, no overflow, no overlap. The standalone files below stay English-only on purpose (English is the language new content is authored in first, per `STANDARDS.md`), the translated inline copies live only inside the two HTML articles.
