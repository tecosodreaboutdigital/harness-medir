...
**Why English.** Per `STANDARDS.md`'s `Languages` rule, new content is authored in English first, so these stayed English-only through drafting. Parts 3 and 4 are both now written: each diagram above has been copied inline into `harness-p3.html` or `harness-p4.html`, re-IDed per language through the build scripts' `scope()` function (`p3d1` through `p3d5`, `p4d6` through `p4d10`). The English inline copies match the standalone files below exactly; Portuguese and Spanish translation of the surrounding prose, and of the diagrams' own labels, is still pending for Part 4 (Part 3's is complete in all three languages).
