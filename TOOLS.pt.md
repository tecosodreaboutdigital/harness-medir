...
**A citação no guia compacto, feita na mesma rodada.** As cinco primeiras coleções foram instaladas e citadas em `harness-toolkit.html` na mesma sessão. Esta rodada separou os dois passos de propósito, para o operador decidir sobre a instalação primeiro, e depois escreveu as fichas de seis campos, nas três línguas, com o espelho em `build/` sincronizado: `humanizer` em Reforçar, o pacote de engenharia de contexto em Equipar, `threat-modeling` em Proteger e `ai-act-skill` em Governar. Este parágrafo continuou dizendo que a citação estava pendente por nove dias depois de ela estar feita, porque foi commitado no mesmo commit das próprias fichas; foi corrigido em 20 de setembro de 2026, conferindo cada documento publicado contra o repositório.
