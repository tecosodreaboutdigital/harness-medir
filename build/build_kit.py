...
p2=open(os.path.join(OUT,'harness-p2.html'),encoding='utf-8').read()
...
BODY=open('/home/claude/body_kit_pt.html',encoding='utf-8').read()
def scope(b,pref):
...
open(os.path.join(OUT,'harness-caixa-de-ferramentas.html'),'w',encoding='utf-8').write(doc)
ids=set(re.findall(r'\sid="([a-z0-9\-]+)"',doc)); hr=set(re.findall(r'href="#([a-z0-9\-]+)"',doc))
