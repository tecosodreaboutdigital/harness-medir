...
**Limite que continua, do reprice.** `tokens_bucket` guarda quatro contagens, sem separar a escrita de cache de 5 minutos da de 1 hora, e o port precifica toda escrita de cache a `cache_creation_price` (US$2,50 por milhão). Nas transcrições deste projeto, todas as escritas de cache lidas (12.301.931 de 12.301.931 tokens) são de 1 hora, a US$4,00. O custo gravado provavelmente subestima esse componente em cerca de US$4,82 nos onze marcos (3.211.389 tokens vezes a diferença de US$1,50 por milhão), uma estimativa, não uma medição por marco. A skill de origem já separa os dois TTLs e precifica cada modelo com a própria série; este port não, e portar essa divisão pede guardar a divisão em cada marco novo, o que não foi feito.
