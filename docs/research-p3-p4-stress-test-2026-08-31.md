...
**A later link-and-URL audit of everything this dossier led to found one real error in its own Axis 6 finding, corrected the same day.** The claim that Microsoft's Agent Governance Toolkit "automates decommissioning with no owner or certifier sign-off required" overstated what the source code actually does. Read directly (`models.py`, `manager.py`, `orphan_detector.py`, `credentials.py`, commit `46463ef`): the toolkit automatically detects decommission and orphan candidates by an inactivity threshold and a heartbeat timeout, but the functions that actually change state, `LifecycleManager.decommission()` and `OrphanDetector.mark_orphaned()`, are separate, explicit calls that nothing in the toolkit invokes on their own. Those functions also do not themselves check for an approval record, so a team wiring its own automation straight from the candidate list to the decommission call would get the ungated failure this series warns against, but the toolkit as shipped does not do that wiring itself. Corrected everywhere the original claim appeared, framed as an empty hook rather than a shipped failure, which better serves the section's own argument than the overstatement did. One separately confirmed automatic behaviour survived the correction: credential rotation (`max_credential_ttl`, `CredentialRotator.check_and_rotate()`) does run automatically with no approval check, just not the decommissioning transition this series specifically discusses. The open date (2 April 2026) and star count (3,300+) were also stale; corrected to 2 March 2026 and over 6,000 stars, confirmed via the GitHub API.
